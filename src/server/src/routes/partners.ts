...
import { type Partner, partnersModel } from "./partners.model";

export const partnersRoute = new Elysia({
	prefix: "/partners",
	tags: ["Partners"],
})
	.model(partnersModel)
	...
	.get(
		"",
		async ({
			query: { page, pageSize, sortBy, sortOrder, search, name, isActive },
		}) => {
			try {
				// 搜索条件构建
				const conditions = [];
				...
				const sortField =
					sortFieldMap[sortBy || "sortOrder"] || partnersSchema.sortOrder;
		...
		async ({ params: { id } }) => {
			try {
				const partner = await db
					.select()
					.from(partnersSchema)
					.where(eq(partnersSchema.id, parseInt(id, 10)))
					.limit(1);
				if (partner.length === 0) {
