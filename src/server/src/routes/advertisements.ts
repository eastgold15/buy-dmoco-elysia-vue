...
export const advertisementsRoute = new Elysia({
	...
	.get(
		...
		async ({
			...
			try {
				...
				const advertisements = await queryBuilder
					.limit(pageSize)
					.offset(offset);
				...
				return pageRes(advertisements, 200, "获取广告列表成功", {
					page,
					pageSize,
					total,
		...
		async ({ params: { id } }) => {
			try {
				const [advertisement] = await db
					.select()
					.from(advertisementsSchema)
					.where(eq(advertisementsSchema.id, parseInt(id, 10)));
	...
	.patch(
		"/:id/sort",
		async ({ params: { id }, body: { sortOrder } }) => {
			try {
				const [advertisement] = await db
					.update(advertisementsSchema)
					.set({
