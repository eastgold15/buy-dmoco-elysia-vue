					...
					async ({ params: { id }, body }) => {
						try {
							const result = await db
								...
								.where(eq(categoriesSchema.id, parseInt(id, 10)))
								.returning();
	...
	.get(
		...
		async ({
			...
			try {
				...
				if (parentId) {
					conditions.push(
						eq(categoriesSchema.parentId, parseInt(parentId, 10)),
					);
				}
	...
	.delete("/:id", async ({ params: { id } }) => {
		try {
			...
			const children = await db
				.select()
				.from(categoriesSchema)
				.where(eq(categoriesSchema.parentId, parseInt(id, 10)))
				.limit(1);
		...
		try {
			...
			const prevCategory = await db
				...
				.where(
					and(
						current.parentId
							? eq(categoriesSchema.parentId, current.parentId)
